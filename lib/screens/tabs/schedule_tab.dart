...
class _ScheduleTabState extends State<ScheduleTab>
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24), // Tab Bar
            Container(
              height: 48,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12), // Route full name with location icon
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Icon(
            ...
            const SizedBox(height: 16), // Time slots
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children:
  ...
  Widget _buildTimeSlot(String time, List<String> allTimes, Color color) {
    ...
    // Convert current time to 12-hour format to match bus times
    String period = now.hour >= 12 ? 'PM' : 'AM';
    int displayHour =
        now.hour == 0 ? 12 : (now.hour > 12 ? now.hour - 12 : now.hour);
    final currentTime =
        '${displayHour}:${now.minute.toString().padLeft(2, '0')} $period';
  ...
  BusStatus _getNextBusStatus(
    ...
    // If bus time is in the past, it's departed
    if (busMinutes < currentMinutes) {
      return BusStatus.departed;
    }

    // If the bus is within 10 minutes and is the next one, mark as current
    final minutesToBus = busMinutes - currentMinutes;
    if (minutesToBus <= 10) {
      // Check if this is the very next bus
      final nextBusMinutes = allTimes
          .map(_timeToMinutes)
          .where((time) => time >= currentMinutes)
          .reduce((a, b) => a < b ? a : b);

      if (busMinutes == nextBusMinutes) {
        return BusStatus.current;
      }
    }
    ...
    // The very next bus is marked as next
    if (busMinutes == futureBuses.first) {
      return BusStatus.next;
    }

    // All other future buses are upcoming
